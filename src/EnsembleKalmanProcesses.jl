...
function EnsembleKalmanProcess(parameters::Array{FT, 2},
    ...
    # Throw an error if an attempt is made to instantiate a `Sampler` EnsembleKalmanProcess
    ...
    EnsembleKalmanProcess{FT, IT, P}(u, parameter_names, obs_mean, obs_noise_cov, N_ens, g,
                     err, Δt, process)
end
...
function compute_error!(ek::EnsembleKalmanProcess)
...
"""
   find_ek_step(ek::EnsembleKalmanProcess{FT, IT, Inversion}, g::Array{FT, 2}; cov_threshold::FT=0.01) where {FT}
...
"""
function find_ek_step(ek::EnsembleKalmanProcess{FT, IT, Inversion}, g::Array{FT, 2}; cov_threshold::FT=0.01) where {FT, IT}
    accept_step = false
    if !isempty(ek.Δt)
...
function update_ensemble!(ek::EnsembleKalmanProcess{FT, IT, Inversion}, g; cov_threshold::FT=0.01, Δt_new=nothing) where {FT, IT}
    # u: N_ens x N_params
    u = ek.u[end]
    cov_init = cov(ek.u[end], dims=1)
...
function update_ensemble!(ek::EnsembleKalmanProcess{FT, IT, Sampler{FT}}, g) where {FT, IT}
    # u: N_ens x N_params
    # g: N_ens x N_data
    u = ek.u[end]
    N_ens = ek.N_ens
